...
class MenuActivity8 : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        applicationContext.getSharedPreferences("MenuActivity8", MODE_PRIVATE)

        // 다음으로 버튼
        menu8_next_btn.setOnClickListener {
            // question_8
            if (radio1_1.isChecked==true){
                SPEditor.putString(question_8, "0").apply()
            }
            else if(radio1_2.isChecked==true){
                SPEditor.putString(question_8, "1").apply()
            }
            else if(radio1_3.isChecked==true){
                SPEditor.putString(question_8, "2").apply()
            }
            else if(radio1_4.isChecked==true){
                SPEditor.putString(question_8, "3").apply()
            }
            else if(radio1_5.isChecked==true){
                SPEditor.putString(question_8, "4").apply()
            }

            // question_9
            if (radio2_1.isChecked==true){
                SPEditor.putString(question_9, "0").apply()
            }
            else if(radio2_2.isChecked==true){
                SPEditor.putString(question_9, "1").apply()
            }
            else if(radio2_3.isChecked==true){
                SPEditor.putString(question_9, "2").apply()
            }
            else if(radio2_4.isChecked==true){
                SPEditor.putString(question_9, "3").apply()
            }
            else if(radio2_5.isChecked==true){
                SPEditor.putString(question_9, "4").apply()
            }
